...
| 卡 | 功能 | 归属 |
|---|---|---|
| F1 | 实时 API 通道 `api_client.dart`（远端优先 + asset 回退） | A |
| F2 | 四份契约 + `LocalGrantRepository` 桩（含假角色开关） | A |
| F3 | Supabase 基建：4 张表 · Auth · RLS | B |
| F4 | 云端档案与收藏合并 + 认证三屏 | B |
| F5 | 资助管理端两屏（含压缩成一屏的预案） | B |
| F6 | 资助用户端三屏 + 资格判定 + Breakdown 接点 | C |
| F7 | 从 `metrics.dart` 拆出 `policy_metrics.dart` | A→D |
| F8 | 政策目录远端化（版本比对 + 回退） | D |
| F9 | 导出 CSV / PDF | D |
...
排序只看两条：**谁在挡别人的路**，和**砍掉后损失最小的排最后**。9/6 提交，倒推如下。

| 批次 | 时间 | 并行做什么 | 这批结束时的状态 |
|---|---|---|---|
| **0 · 解锁** | 即刻（半天内） | **F2**（A）· **F7**（A 搬运）· B 定表结构给 A | C 和 D 拿到能开工的东西，全员不再空等 |
| **1 · 各自开工** | 8/31–9/1 | **F3**（B，硬期限）· **F6 前半**（C，跑在本地桩上）· **F8**（D）· **F1**（A） | 每人手上都有一条真 HTTP 调用跑通了 |
| **2 · 接云与补齐** | 9/1–9/2 | **F4**（B）· **F6 后半**（C 换 Supabase 实现）· **F9**（D） | 登录、收藏合并、资助闭环的用户侧全通 |
| **3 · 管理端与联调** | 9/2–9/3 | **F5**（B）· 全员联调 · 双账号审批演练 | 功能冻结 |
| **4 · 收尾** | 9/3–9/5 | 真机测试 + `flutter analyze` 归零 → 剥注释 → release APK | 可提交 |

**为什么是这个顺序**

- **F2 必须第一个做完**，它是全项目唯一的阻塞点：没有契约和本地桩，C 动不了、D 没有网络模板可抄。半天做完，不要拖成一天。
- **F7 和 F2 同批**，因为它只是搬运，但拖着不做的话 D 每写一行政策代码，`git blame` 都记在 A 名下——这是评分证据，越早拆越干净。
- **F3 排在 F4 / F5 之前**且是 B 的硬期限：B 自己的两块和 C 的换实现都压在它上面，它一晚，后面三件一起晚。
- **F1 排批次 1 而不是批次 0**，因为它谁也不挡；但也不能更晚——它是 A 的网络调用证据。
- **F9 排最后一个功能位**，它是全部九件里唯一砍掉不影响任何人的（分享是锦上添花，且不算网络调用）。真到 9/2 还没动，就砍它，不要砍 F8。

**时间不够时的取舍顺序**（从先砍到死也不能砍）：
F9 导出 → F5 压缩成一屏 → F4 的收藏合并降级为「登录后只读云端」 → **以下不可砍**：F2 · F3 · F6 · F8（砍任何一件都会让某个人失去网络调用或数据处理证据）。
